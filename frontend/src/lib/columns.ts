...
export interface ColDef {
  key: string
  label: string
  type: 'text' | 'number' | 'date' | 'integer'   // input element type in edit mode
  fmt: ColFmt                                       // display format in read mode
  readOnly?: boolean    // computed fields — shown but not editable
  frozen?: boolean      // sticky-left column
  width?: string        // Tailwind min-w class
  rightAlign?: boolean  // numeric columns should be right-aligned
}
...
// ── Cambridge (16 data cols + notes) ─────────────────────────
export const CAMBRIDGE_COLS: ColDef[] = [
  { key: 'enbridge_qtr_reference',          label: 'Qtr Ref',               type: 'text',    fmt: 'text',    frozen: true, width: 'min-w-28' },
  { key: 'start_date',                      label: 'Start Date',             type: 'date',    fmt: 'date',    frozen: true, width: 'min-w-32' },
  { key: 'end_date',                        label: 'End Date',               type: 'date',    fmt: 'date',    width: 'min-w-32' },
  { key: 'billing_period',                  label: 'Billing Period',         type: 'date',    fmt: 'date',    width: 'min-w-32' },
  { key: 'cd',                              label: 'CD (m³)',                type: 'number',  fmt: 'volume',  width: 'min-w-24', rightAlign: true },
  { key: 'gas_consumption',                 label: 'Gas Consumption (m³)',   type: 'number',  fmt: 'volume',  width: 'min-w-36', rightAlign: true },
  { key: 'split_volumes',                   label: 'Split Volumes (m³)',     type: 'text',    fmt: 'text',    width: 'min-w-32' },
...
// ── Pickering CNG (22 data cols + notes) ─────────────────────
export const PICKERING_CNG_COLS: ColDef[] = [
  ...
  { key: 'cd',                         label: 'CD (m³)',                type: 'text',    fmt: 'text',    width: 'min-w-32' },
  { key: 'gas_consumption',            label: 'Gas Consumption (m³)',   type: 'number',  fmt: 'volume',  width: 'min-w-36', rightAlign: true },
  { key: 'split_volumes',              label: 'Split Volumes (m³)',     type: 'text',    fmt: 'text',    width: 'min-w-32' },
...
// ── Walgreen (20 data cols + notes) ──────────────────────────
export const WALGREEN_COLS: ColDef[] = [
  { key: 'enbridge_qtr_reference',     label: 'Qtr Ref',                    type: 'text',    fmt: 'text',    frozen: true, width: 'min-w-32' },
...
// ── Elexicon (24 data cols + notes) ──────────────────────────
export const ELEXICON_COLS: ColDef[] = [
  { key: 'bill_period',                      label: 'Bill Period',            type: 'text',    fmt: 'text',    frozen: true, width: 'min-w-24' },
  ...
  // ← Distribution Charges group starts at index 9
  ...
  // ← Distribution Charges ends. Other Charges group starts at index 13
  ...
  // ← Other Charges ends at index 19
...
// Elexicon column groups (0-indexed into ELEXICON_COLS)
export const ELEXICON_GROUPS: ColGroup[] = [
  { label: 'Distribution Charges', startIndex: 9,  colSpan: 4 },
  { label: 'Other Charges',        startIndex: 13, colSpan: 7 },
]
...
export function buildGroupHeaderSpec(
  ...
  while (i < cols.length) {
    ...
    if (grp) {
      ...
      // Accumulate contiguous ungrouped columns
